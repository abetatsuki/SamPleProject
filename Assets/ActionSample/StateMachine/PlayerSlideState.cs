...
namespace ActionSample.StateMachine
{
    ...
    public class PlayerSlideState : PlayerState
    {
        ...
        public override void Enter()
        {
            ...
            // スケール変更（しゃがみ動作）
            // スライディング時の姿勢を視覚的に表現するため
            _originalScale = Context.transform.localScale;
            Context.transform.localScale = new Vector3(_originalScale.x, Context.SlideYScale, _originalScale.z);

            // スライディング方向の決定
            // 移動入力がある場合はその方向へ、入力がない場合は向いている方向（前方）へ滑らせるため
        ...
        public override void Exit()
        {
            ...
            // スケールを元に戻す
            // スライディング終了後に通常の立ち姿勢に戻すため
            Context.transform.localScale = _originalScale;

            // 空気抵抗をリセット
            // 通常移動（歩行など）に戻った際に、スライディング用の減速設定を引き継がないようにするため
        ...
        public override void PhysicsUpdate()
        {
            ...
            // スライディング終了判定
            // 設定された時間が経過したら、次のアクション（歩行または待機）へ遷移させるため
            if (_slideTimer <= 0f)
            {
